page 50558 "Dispatch Note List"
{
    ApplicationArea = Basic, Suite, Assembly;
    Caption = 'Dispatch Note';
    CardPageID = "Dispatch Note Card";
    ...
    SourceTableView = WHERE ("Document Type" = CONST (Order), "Dispatch Note" = const (true), "Completely Shipped" = const (false));
    UsageCategory = Lists;
        ...
        area(Creation)
        {
            action(New)
            {
                ...
                begin
                    ...
                    NewNoSeries := NoSeriesManagement.GetNextNo(SalesReceivablesSetup."Dispatch Note No. Series", TODAY, TRUE);

                    SalesHeaderG.RESET;
                    SalesHeaderG.INIT;
                    SalesHeaderG."Document Type" := SalesHeaderG."Document Type"::Order;
                    SalesHeaderG."No." := NewNoSeries;
                    SalesHeaderG."Dispatch Note" := true;
                    SalesHeaderG.INSERT;
                    COMMIT;
                    PAGE.RUNMODAL(50557, SalesHeaderG);
                end;
