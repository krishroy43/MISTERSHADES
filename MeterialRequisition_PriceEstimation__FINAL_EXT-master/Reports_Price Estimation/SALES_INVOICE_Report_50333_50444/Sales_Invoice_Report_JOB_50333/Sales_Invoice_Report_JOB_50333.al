report 50333 "Sales Invoice Report_JOB"
{
    DefaultLayout = RDLC;
    Caption = 'Sales Invoice Report JOB';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            dataitem("Sales Invoice Line"; "Sales Invoice Line")
            {
                ...
                dataitem("Cust. Ledger Entry"; "Cust. Ledger Entry")
                {
                    ...
                    begin
                        ...
                        "Cust. Ledger Entry".setfilter("Entry No.", '<%1', EntryNo);
                        // Stop 05-09-2019
                    ...
                    begin
                        if "Document No." <> ' ' then
                            TotalInvoiceAmountWithoutTax += (Amount - (Amount * "Sales Invoice Line"."VAT %") / (100 + "Sales Invoice Line"."VAT %"));
                    end;
                ...
                dataitem(BodyLoop_PaymentMilestone; "Payment Milestone")
                {
                    ...
                    OnPreDataItem()
                    begin
                        if not "Sales Invoice Header"."Progressive Invoice" then
                            Setfilter("Document No.", '=%1', "Sales Invoice Line"."Job No.")
                        else
                            Setfilter("Document No.", '=%1', "Sales Invoice Line"."Document No.");
            ...
            begin
                ...
                if (salesInvoiceLine.FindFirst()) and (salesInvoiceHeader = 0) then begin
                    if salesInvoiceLine."Job No." <> '' then begin
                        FinalAmountIncludingVAT := FindFinalAmount__RDLC(salesInvoiceLine."Document No.");
                        //FinalAmountIncludingVAT := FindFinalAmount(salesInvoiceLine."Document No.");
                        ...
                        //Final_AmountIncludingVAT
                        //AmountInWordCodeunitCU.FormatNoText(AmountInWordForJob, Final_AmountIncludingVAT, CurrCode);
    ...
    begin
        ...
        Clear(VATAmountDec);
        if SalesInvoiceHeader_Rec.Get(DocNo) then begin
    ...
    begin
        ...
        if SalesInvoiceHeader_Rec.Get(DocNo) then begin
            SalesInvoiceLine_Rec.Reset();
            SalesInvoiceLine_Rec.SetRange("Document No.", SalesInvoiceHeader_Rec."No.");
            //  SalesLineRec.SetRange("Document Type", SalesInvoiceHeader_Rec."Document Type");
            if SalesInvoiceLine_Rec.FindFirst() then begin
                // Stop Sales Header               
                CustLedgEntryL.reset;
                CustLedgEntryL.SetRange("Document No.", "Sales Invoice Header"."No.");
                ...
                //if "Sales Invoice Header"."Progressive Invoice" then
                //    CustLedgerEntry.SetRange("Job No.", SalesInvoiceLine_Rec."Document No.")
                //else
                ...
                if CustLedgerEntry.FindSet() then
                    repeat
                        if CustLedgerEntry."Document No." <> '' then begin
                            ...
                            TotalInvoiceAmountWithoutTax_Val += (CustLedgerEntry.Amount - (CustLedgerEntry.Amount * SalesInvoiceLine_Rec."VAT %") / (100 + SalesInvoiceLine_Rec."VAT %"));
                        end;
                ...
                if "Sales Invoice Header"."Progressive Invoice" then
                    PaymentMilestoneRecL.SetRange("Document No.", SalesInvoiceLine_Rec."Document No.")
                else
