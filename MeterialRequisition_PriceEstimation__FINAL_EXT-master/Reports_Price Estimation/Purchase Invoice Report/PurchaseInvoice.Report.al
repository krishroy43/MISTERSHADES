report 50016 "Purchase Invoice Ext"
{
    DefaultLayout = RDLC;
        ...
        dataitem("Purch. Inv. Header"; "Purch. Inv. Header")
        {
            ...
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem(PageLoop; Integer)
                {
                    ...
                    //{
                    //}
                    column(OrderNo_PurchInvHeader; LastOrderNo)
                    {
                    }
            ...
            begin
                PurchInvLineL.reset;
                PurchInvLineL.SetRange("Document No.", "Purch. Inv. Header"."No.");
                PurchInvLineL.SetFilter("Receipt No.", '<>%1', '');
                if PurchInvLineL.findset then
                    repeat
                        PurchRcptHeader.reset;
                        PurchRcptHeader.SetRange("No.", PurchInvLineL."Receipt No.");
                        if PurchRcptHeader.FindFirst() then begin
                            If LastOrderNo = '' then
                                LastOrderNo := PurchRcptHeader."Order No.";
                        end;
